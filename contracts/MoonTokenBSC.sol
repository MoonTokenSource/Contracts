...
contract SaveTheMoon is Context, IERC20, Ownable {
    ...
    constructor(address _bridgingAddress, address _marketingAddress) {
    ...
    function setBridgingAddress(address newAddress) public onlyOwner {
    ...
    function _transferBothExcluded(
        ...
        if (isSell) {
            ...
            _takeToBuyLuna(tLunaOrBurn);
        }
        _takeToMarketing(tMarketing);
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLunaOrBurn).sub(
            tMarketing
        );
        return (tTransferAmount, tFee, tLunaOrBurn, tMarketing);
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLunaOrBurn).sub(
            rMarketing
        );
    ...
    function _takeToBuyLuna(uint256 tLunaOrBurn) private {
        uint256 currentRate = _getRate();
        uint256 rLunaOrBurn = tLunaOrBurn.mul(currentRate);
        _rOwned[bridgingAddress] = _rOwned[bridgingAddress].add(rLunaOrBurn);
        if (_isExcluded[bridgingAddress])
