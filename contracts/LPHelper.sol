...
contract AddLiquidityHelper is ReentrancyGuard, Ownable {
    ...
    function addMOONETHLiquidity(uint256 nativeAmount) external payable nonReentrant {
        ...
        ERC20(address(_moon)).safeTransferFrom(msg.sender, address(this), nativeAmount);

        // approve token transfer to cover all possible scenarios
        ERC20(address(_moon)).approve(address(mooonSwapRouter), nativeAmount);
        ...
        uint256 mooonBalance = ERC20(address(_moon)).balanceOf(address(this));
    ...
    function addTokenMOONLiquidity(address baseTokenAddress, uint256 baseAmount, uint256 nativeAmount) external nonReentrant {
        ...
        // approve token transfer to cover all possible scenarios
        ERC20(baseTokenAddress).approve(address(mooonSwapRouter), baseAmount);
        ...
        if (ERC20(address(_moon)).balanceOf(address(this)) > 0)
            ERC20(address(_moon)).transfer(msg.sender, ERC20(address(_moon)).balanceOf(address(this)));
    }
